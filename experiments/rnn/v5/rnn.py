...
class RNN:
    ...
    def constructModel(self):
        ...
        self.hiddenLayer = MultiInputLayer( nins=[self.nin, self.n],
                                         nout=self.n,
                                         activation=T.nnet.sigmoid,
                                         name='hiddenLayer')

        # the hidden layer output goes to the output
        self.outputLayer = Layer( nin=self.n,
                             nout=self.nout,
                             activation=T.nnet.sigmoid,
                             name='outputLayer')
        ...
        def step(x_t, h_tm1):
            h_t = self.hiddenLayer.compute([x_t, h_tm1])
            y_t = self.outputLayer.compute(h_t)
            return h_t, y_t
